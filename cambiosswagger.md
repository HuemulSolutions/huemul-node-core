# Cambios en `huemul-node-core`: generador OpenAPI/Swagger

> Documento autocontenido para ejecutar en el repo de **`@huemulsolutions/huemul-node-core`** (no requiere contexto externo).

## Objetivo

Agregar a core un generador de OpenAPI **puro** que, a partir de los `IHuemulColumnDef[]` que ya exporta cada módulo (`<module>ColumnsInfo`), produzca el documento OpenAPI completo: schemas por módulo, los 6 paths CRUD estándar, endpoints custom (`extra`) y los componentes del envoltorio de respuesta `HuemulLog`.

**Principios:**
- **Sin dependencias nuevas.** Todo devuelve JSON plano (objetos OpenAPI 3.0). No se importa `express` ni `swagger-ui-express`: servir la UI es responsabilidad de cada app consumidora.
- Reutiliza tipos que ya viven en core: `IHuemulColumnDef` y `huemulBaseColumnsInfo`.

Tipo base ya existente en core (`src/interfaces/interface-huemul-column-def.ts`):
```ts
interface IHuemulColumnDef { columnName; columnType; columnDescription; pkType; allowNull;
  required; numOrderInGet; columnPosition; columnSubType?; columnLength?; columnPrecision?;
  PKModuleNameId?; PKModuleName?; versionRelease? }   // NO incluye PKModuleAliasColumnName
```
> Los archivos generados en las apps agregan `PKModuleAliasColumnName` vía cast `as IHuemulColumnDef`. Por eso, si se necesita, se lee con acceso laxo `(col as any).PKModuleAliasColumnName` y nunca se asume presente en el tipo.

---

## 1. Nuevo archivo: `src/swagger/huemul-swagger.ts`
...
- **Sin dependencias nuevas** (todo es JSON plano; los tipos OpenAPI se expresan con objetos).
- Subir versión **`1.0.5` → `1.1.0`** (feature menor, retrocompatible).

---

## 4. Tests (vitest) — `src/swagger/huemul-swagger.test.ts`

Cubrir:
- `huemulColumnToOpenApi`: un caso por `columnType` — `string` (con `maxLength`), `number` sin precision (`integer`), `number` con `columnPrecision>0` (`number/double`), `boolean`, `date`, `timestamp`, `picker` (prefijo `[picker]`), tipo desconocido (`string`); y FK (`PKModuleName` no vacío → description con `(FK -> ...)`).
- `buildModuleSchemas`: el schema `Body` excluye columnas de `huemulBaseColumnsInfo` pero **conserva `versionKey`**; `required[]` incluye solo columnas con `required && !allowNull`.
- `buildCrudPaths`: existen las 6 rutas (`/api{prefix}/v1/` post/put/get, `/api{prefix}/v1/{pk}` get/delete, `/api{prefix}/multi/delete/v1/` put), el param de path usa `pkName`, y el body de multi-delete pide `<module>IdList`.
- `buildExtraPaths`: `multipart` genera `multipart/form-data` con `file: binary`; `bodySchema` genera `application/json`.
- `buildHuemulOpenApiPaths`: módulo cuyo `loader` devuelve `undefined` → `console.warn` y continúa; respeta `standardCrud:false` (no emite CRUD pero sí `extra`).

Ejemplo de un test:
```ts
import { describe, it, expect } from "vitest";
import { huemulColumnToOpenApi } from "./huemul-swagger";

describe("huemulColumnToOpenApi", () => {
  it("number con precision → double", () => {
    const p = huemulColumnToOpenApi({ columnType: "number", columnPrecision: 4 } as any);
    expect(p).toMatchObject({ type: "number", format: "double" });
  });
  it("FK agrega descripción", () => {
    const p = huemulColumnToOpenApi({
      columnType: "string", columnDescription: "ref", PKModuleName: "gcClient", PKModuleNameId: "gcClientId",
    } as any);
...
npm publish       # registry GitHub Packages (npm.pkg.github.com)
```
`files: ["dist"]` ya incluye `dist/swagger/` y sus `.d.ts`.

---

## 6. Contrato que consumirá la app (no cambiar firmas sin avisar)

```ts
import {
  buildHuemulOpenApiPaths, huemulResponseComponents, HuemulSwaggerModuleDef,
...
// app sirve la UI con swagger-ui-express (core NO lo hace)
```

> **Importante para la app:** los `schemas` de core (`HuemulResponse`) y los de los módulos deben **fusionarse** (`{ ...core.schemas, ...schemas }`), no sobrescribirse. Las `responses` reutilizables van bajo `components.responses`.
